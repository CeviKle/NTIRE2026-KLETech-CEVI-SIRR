...
## 12.1. Validation and Testing Configuration Strategy

We follow the same approach for both validation and testing:

- We **do not modify the original training YAML**
- Separate YAML config files are created for:
  - Validation
  - Testing
  - Checkpoint-specific evaluation

Just like validation configs (e.g., `train_rdnet_val300.yml`),  
testing configs (e.g., `test_109.yml`) are created similarly.
...
## 12.2. Note on Validation / Testing Setup

- No strict ground truth available
- Pseudo targets used for comparison
- PSNR is **relative, not absolute**
